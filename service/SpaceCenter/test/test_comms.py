...
class TestCommsSingleHop(krpctest.TestCase):
    ...
    def test_power(self):
        ...
        exponent = sum(
            power * exponent for power, exponent in zip(powers, exponents)
        ) / sum(powers)
        power = max(powers) * sum(power / max(powers) for power in powers) ** exponent
    ...
    def test_control_path(self):
        ...
        self.assertAlmostEqual(self.signal_strength, link.signal_strength, places=2)
        # Start is vessel
        start = link.start
