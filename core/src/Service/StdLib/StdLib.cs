...
namespace KRPC.Service.StdLib
{
    ...
    public static class StdLib
    {
        ...
        /// <summary>
        /// The given number rounded to the given number of decimal places,
        /// with a halfway value rounded to the nearer even number.
        /// </summary>
        /// <param name="x"></param>
        /// <param name="digits">The number of decimal places to round to.
        /// Negative to round to tens, hundreds and beyond.</param>
        [KRPCProcedure]
        public static double Round (double x, int digits = 0)
        {
            return RoundToDigits (x, digits, MidpointRounding.ToEven);
        }

        /// <summary>
        /// The given number rounded to the given number of decimal places,
        /// with a halfway value rounded away from zero.
        /// </summary>
        /// <param name="x"></param>
        /// <param name="digits">The number of decimal places to round to.
        /// Negative to round to tens, hundreds and beyond.</param>
        [KRPCProcedure]
        public static double RoundHalfAwayFromZero (double x, int digits = 0)
        {
            return RoundToDigits (x, digits, MidpointRounding.AwayFromZero);
        }

        /// <summary>
        /// The given number rounded to the given number of decimal places,
        /// with a halfway value rounded toward positive infinity.
        /// </summary>
        /// <param name="x"></param>
        /// <param name="digits">The number of decimal places to round to.
        /// Negative to round to tens, hundreds and beyond.</param>
        [KRPCProcedure]
        public static double RoundHalfUp (double x, int digits = 0)
        {
        ...
        // A double carries fewer significant digits than this, so rounding to more
        // decimal places than it leaves the value alone
        const int MaxRoundingDigits = 15;

        /// <summary>
        /// Rounding to a number of decimal places. Math.Round takes nought to fifteen
        /// of them, so scale the value to round it to tens, hundreds and beyond.
        /// </summary>
        static double RoundToDigits (double x, int digits, MidpointRounding mode)
        {
            if (digits > MaxRoundingDigits)
                return x;
            if (digits >= 0)
                return Math.Round (x, digits, mode);
            // A power of ten is exact where its reciprocal is not, so divide by it
            var scale = Math.Pow (10, -digits);
            if (double.IsInfinity (scale))
                return 0;
            return Math.Round (x / scale, mode) * scale;
        }
        ...
        /// <summary>
        /// The logarithm of a number, natural unless another base is given.
        /// </summary>
        /// <param name="x"></param>
        /// <param name="newBase">The base of the logarithm.</param>
        [KRPCProcedure]
        public static double Log (double x, double newBase = Math.E)
        {
        ...
        {
            var sqrMagnitude =
                q.Item1 * q.Item1 + q.Item2 * q.Item2 + q.Item3 * q.Item3 + q.Item4 * q.Item4;
            if (sqrMagnitude <= 0)
                throw new ArgumentException ("Cannot invert a zero quaternion");
            return new Tuple4 (
                -q.Item1 / sqrMagnitude, -q.Item2 / sqrMagnitude,
                -q.Item3 / sqrMagnitude, q.Item4 / sqrMagnitude);
        }
