    ...
    static class TypeUtils
    {
        ...
        public static string GetTypeName (Type type)
        {
            if (!IsAValidType (type))
                ...
                return ProtocolBuffers.GetMessageTypeName (typeof(Krpc.List));
            else if (IsADictionaryCollectionType (type))
                return ProtocolBuffers.GetMessageTypeName (typeof(Krpc.Dictionary));
            else if (IsASetCollectionType (type))
                return ProtocolBuffers.GetMessageTypeName (typeof(Krpc.Set));
            else if (IsATupleCollectionType (type))
                return ProtocolBuffers.GetMessageTypeName (typeof(Krpc.Tuple));
            else
                throw new ArgumentException ("Type is not valid");
        }
