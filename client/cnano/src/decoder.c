...
static bool krpc_return_value_decoder(pb_istream_t *stream, const pb_field_t *field, void **arg) {
  ...
  if (!pb_read(stream, buffer, size)) KRPC_CALLBACK_RETURN_ERROR("decoding return value");
  result->size = size;
...
krpc_error_t krpc_decode_string(pb_istream_t *stream, char **value) {
  ...
  if (!*value) *value = (char *)krpc_malloc(length + 1);
  if (!pb_read(stream, (pb_byte_t *)(*value), length))
    KRPC_RETURN_STREAM_ERROR(DECODING_FAILED, "failed to decode value of type string", stream);
  *((*value) + length) = 0;
  return KRPC_OK;
}
...
krpc_error_t krpc_decode_bytes(pb_istream_t *stream, krpc_bytes_t *value) {
  ...
  if (value->size > 0) {
    ...
    if (!buffer) KRPC_RETURN_ERROR(DECODING_FAILED, "failed to allocate buffer for krpc_bytes_t");
    value->size = size;
    value->data = buffer;
  }
  if (!pb_read(stream, buffer, size))
    KRPC_RETURN_STREAM_ERROR(DECODING_FAILED, "failed to decode value of type krpc_bytes_t",
                             stream);
  return KRPC_OK;
}
