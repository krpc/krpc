...
class _Compiler:
        ...
        if isinstance(func, ast.Attribute):
            ...
            if not base.is_value and base.ptype is not None:
                ...
                if mutation is not None:
                    return mutation
            if self._is_a_string(base) and (
                not base.is_value
                or any(not self._compile(arg).is_value for arg in node.args)
            ):
                string_method = self._compile_string_method(node, func.attr, base)
                if string_method is not None:
    ...
    # collection it is called on, and the number of arguments it takes. Removing
    # reports whether the value was there, and clearing produces no value
    _COLLECTION_METHODS = {
    ...
    def _compile_string_method(
        self, node: ast.Call, name: str, base: _Result
    ) -> Optional[_Result]:
        """Compile a method call on a string, such as upper() or split()."""
        entry = _STRING_METHODS.get(name)
        if entry is None:
            return None
        operation, arity = entry
        if len(node.args) != arity or node.keywords:
        ...
        args = [self._to_expression(self._compile(arg)).expression for arg in node.args]
    ...
    def _compile_boolop(self, node: ast.BoolOp) -> _Result:
        ...
        if all(result.is_value for result in results):
            ...
            return _Result(value=value, is_value=True)
        op = (
            self._expr.conditional_and
            if isinstance(node.op, ast.And)
            else self._expr.conditional_or
        )
    ...
    def _compile_compare(self, node: ast.Compare) -> _Result:
        ...
        if all(operand.is_value for operand in operands):
            ...
            return _Result(value=value, is_value=True)
        declared, prelude = self._hoist_chained_operands(node, operands)
        comparisons = []
        for left, op, right in zip(operands, node.ops, operands[1:]):
            if isinstance(op, (ast.In, ast.NotIn)):
                container = self._to_expression(right)
                operation = (
    ...
    def _hoist_chained_operands(
        ...
        """Assign the middle operands of a chained comparison to temporaries, and
        replace them in place with the temporary. Each stands in two comparisons,
        and python evaluates it once, before the comparisons are combined."""
        declared: List[Any] = []
        prelude: List[Any] = []
    ...
    def _compile_slice(self, node: ast.Subscript) -> _Result:
        ...
        if self._is_a_string(collection):
            return self._compile_string_slice(collection, lower, upper)
    ...
    def _compile_string_slice(
        self, string: _Result, lower: Optional[_Result], upper: Optional[_Result]
    ) -> _Result:
        """Compile a slice of a string. A substring is taken by start and length,
        so an absent bound is the start of the string or the rest of it."""
        expr = self._expr
        start = (
            expr.constant_int(0)
            if lower is None
            else self._to_expression(lower).expression
        )
        if upper is None:
            length = expr.subtract(expr.string_length(string.expression), start)
        else:
            length = self._to_expression(upper).expression
            if lower is not None:
                length = expr.subtract(length, start)
        return _Result(
            expression=expr.string_substring(string.expression, start, length),
            ptype=build_ptype(KRPC.Type.STRING),
        )
        ...
        try:
            ...
            if generator.ifs:
                ...
                for extra in generator.ifs[1:]:
                    ...
                    condition = _Result(
                        expression=self._expr.conditional_and(
    ...
    def _compile_nested_comprehension(
        ...
        try:
            ...
            for condition_node in generator.ifs:
                ...
                collection_expression = self._expr.where(
                    collection_expression,
                    self._expr.lambda_([parameter], condition.expression),
                )
            ...
            return (
                self._expr.select_many(
                    collection_expression,
                    self._expr.lambda_([parameter], inner),
                ),
                inner_element,
            )
        finally:
    ...
    def _compile_builtin(
        ...
        if func is any or func is all:
            ...
            predicate = self._expr.lambda_([identity], identity)
            return _Result(
    ...
    def _compile_round(self, node: ast.Call) -> _Result:
        ...
        if value.ptype is not None and value.ptype.code in _INTEGER_CODES:
            return _Result(
                expression=self._expr.cast(rounded.expression, self._type.int()),
                ptype=build_ptype(KRPC.Type.SINT32),
            )
    ...
    def _converted_expression(
        ...
        if (
            ...
            if target.code == KRPC.Type.SINT32:
                return self._to_expression(result).expression
