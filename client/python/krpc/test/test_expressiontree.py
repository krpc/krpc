...
CHAINED_COMPARISON = """\
Block<Boolean> (chained comparison#0)
  Assign<Int32>
    Parameter<Int32> chained comparison#0
    Block<Int32>
      Call<Void> static Services.CheckExpressionGameScene
        Constant<ProcedureSignature> TestService.TestClass_get_IntProperty
      Call<Int32> TestClass.get_IntProperty
...
class TestExpressionTree(ServerTestCase, unittest.TestCase):
    ...
    def dump(self, func):
        expression = self.conn.compile_function(func)
    ...
    def test_chained_comparison(self):
        # The middle operand is read once, into a temporary, and the comparisons
        # short-circuit
        obj = self.conn.test_service.create_test_object("golden")
        self.check(
            CHAINED_COMPARISON,
            lambda: 0 < obj.int_property < 10,
        )
