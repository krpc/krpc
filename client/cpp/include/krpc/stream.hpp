...
template <typename T>
inline Stream<T>::Stream() : impl(nullptr), acquired(false) {}

template <typename T>
inline Stream<T>::Stream(Client* client, const schema::ProcedureCall& call)
    : impl(client->add_stream(call)), acquired(false) {}

template <typename T>
inline Stream<T>::Stream(Client* client, uint64_t id)
    : impl(client->get_stream(id)), acquired(false) {}

template <typename T>
inline void Stream<T>::start(bool wait) {
  check_exists();
  if (impl->has_started()) return;
  if (!wait) {
    impl->start();
  } else {
...
template <typename T>
inline float Stream<T>::rate() {
  check_exists();
  return impl->rate();
}

template <typename T>
inline void Stream<T>::set_rate(float value) {
  check_exists();
  impl->set_rate(value);
}
...
template <typename T>
inline std::condition_variable& Stream<T>::get_condition() const {
  check_exists();
  return impl->get_condition();
}

template <typename T>
inline std::unique_lock<std::mutex>& Stream<T>::get_condition_lock() const {
  check_exists();
  return impl->get_condition_lock();
}

template <typename T>
inline void Stream<T>::acquire() {
  check_exists();
  acquired = true;
...
template <typename T>
inline void Stream<T>::wait(double timeout) {
  check_exists();
  if (!impl->has_started()) impl->start();
  if (timeout < 0) {
    impl->get_condition().wait(impl->get_condition_lock());
  } else {
...
template <typename T>
inline void Stream<T>::remove_callback(int tag) {
  check_exists();
  impl->remove_callback(tag);
}

template <typename T>
inline void Stream<T>::remove() {
  if (!impl) return;
  impl->remove();
  impl = nullptr;
}
...
template <typename T>
inline bool Stream<T>::operator==(const Stream<T>& rhs) const {
...
template <typename T>
inline bool Stream<T>::operator!=(const Stream<T>& rhs) const {
  if (!impl) return rhs.impl.operator bool();
  if (!rhs.impl) return true;
  return impl->get_id() != rhs.impl->get_id();
}

template <typename T>
Stream<T>::operator bool() const {
  return impl.operator bool();
}

template <typename T>
inline void Stream<T>::check_exists() const {
  if (!impl) throw StreamError("Stream does not exist or was removed");
}
