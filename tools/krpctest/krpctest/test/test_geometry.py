...
class TestGeometry(krpctest.TestCase):
    ...
    def test_normalize(self):
        ...
        self.assertAlmostEqual(
            (1 / math.sqrt(2), 1 / math.sqrt(2), 0), normalize((1, 1, 0))
        )
        self.assertAlmostEqual(
            (1 / math.sqrt(2), 0, 1 / math.sqrt(2)), normalize((1, 0, 1))
        )
    ...
    def test_quaternion_vector_mult(self):
        self.assertAlmostEqual(
            (1, 0, 0), quaternion_vector_mult((0, 0, 0, 1), (1, 0, 0))
        )
        self.assertAlmostEqual(
            (0, 1, 0), quaternion_vector_mult((0, 0, 0, 1), (0, 1, 0))
        )
        self.assertAlmostEqual(
            (0, 0, 1), quaternion_vector_mult((0, 0, 0, 1), (0, 0, 1))
        )
