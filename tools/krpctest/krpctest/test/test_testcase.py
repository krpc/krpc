...
class TestTestCase(TestCase):
    ...
    def test_not_almost_equal_iterable(self):
        self.check_fails(self.assertNotAlmostEqual, (1, 2, 3), (1, 2, 3), delta=0)
        self.assertNotAlmostEqual((1, 2, 3), (3, 3, 4), delta=1)

    def test_almost_equal_dict(self):
        self.assertAlmostEqual({'foo': 1, 'bar': 2}, {'foo': 1, 'bar': 2}, delta=0)
        self.assertAlmostEqual({'foo': 1, 'bar': 3}, {'foo': 1, 'bar': 2}, delta=2)

    def test_not_almost_equal_dict(self):
        self.check_fails(self.assertAlmostEqual, {'foo': 1, 'bar': 2}, {'baz': 3}, delta=0)
        self.assertNotAlmostEqual({'foo': 1, 'bar': 2}, {'foo': 1, 'bar': 4}, delta=1)
    ...
    def test_degrees_almost_equal(self):
        ...
        for case in fail_cases:
            self.check_fails(self.assertDegreesAlmostEqual, case[0], case[1], delta=case[2])
