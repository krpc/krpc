...
def _csc_args(srcs, deps, exe = None, lib = None, doc = None, optimize = True, warn = 4, nowarn = [], define = [], warnaserror = True):
    ...
    if exe:
        args.append("-out:%s" % exe.path)
    if lib:
        args.append("-out:%s" % lib.path)
    if doc:
        args.append("-doc:%s" % doc.path)
    args.append("-warn:%d" % warn)
    if warnaserror:
        args.append("-warnaserror+")
    if len(nowarn) > 0:
        args.append("-nowarn:%s" % ",".join(nowarn))
    if len(define) > 0:
        args.append("-define:%s" % ",".join(define))
...
def _bin_impl(ctx):
    ...
    # for dep in outputs:
    #     sub_commands.append("ln -f -r -s %s %s/%s/%s" % (dep.path, runfile_dir, tmp_dir, dep.basename))
    # for dep in ctx.files.deps:
    #     sub_commands.append("ln -f -r -s $0.runfiles/_main/%s %s/%s/%s" % (dep.short_path, runfile_dir, tmp_dir, dep.basename))
...
def _nuget_package_impl(ctx):
    nuspec = ctx.actions.declare_file(
        ctx.attr.assembly.lib.basename.replace(".dll", ".nuspec"),
        sibling = ctx.attr.assembly.lib,
    )
    assemblies = {
