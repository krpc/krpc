...
def gensrccsharp_impl(ctx):
  out = ctx.outputs.srccsharp
  proto_output = out.path + ".proto_output"
  proto_compiler = ctx.file._proto_compiler
  sub_commands = [
    "rm -rf " + proto_output,
    "mkdir " + proto_output,
    ' '.join([proto_compiler.path, "--csharp_out=" + proto_output,
              ctx.file.src.path]),
    "touch -t 198001010000 $(find " + proto_output + ")",
    "cp " + proto_output + "/*.cs " + out.path, #TODO: don't use shell pattern matching - could be more than one file
  ]
...
gensrccsharp = rule(
  ...
  outputs={"srccsharp": "%{name}.cs"},
)

def proto_csharp_library(name, src):
  gensrccsharp(name=name, src=src)
